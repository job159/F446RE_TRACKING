...
{
  if (s->mode != MODE_IDLE) return "-";
  return (s->idle_substate == IDLE_CALIBRATING) ? "CAL" : "WAIT";
}
...
void Telemetry_Task(Telemetry_HandleTypeDef *h, const TelemetrySnapshot_t *snap)
{
  ...
  unsigned stg = (snap->manual_stage_valid) ? snap->manual_stage : 255;

  char buf[TX_BUF_SIZE];
  int len = snprintf(buf, sizeof(buf),
      "%lu mode:%s sub:%s cal:%u valid:%u "
      "adc:%u,%u,%u,%u base:%u,%u,%u,%u d:%u,%u,%u,%u "
      "err:%ld,%ld cmd:%ld,%ld stg:%u\r\n",
      (unsigned long)h->seq, mode_str(snap->mode), sub_str(snap),
      snap->calibration_done, snap->source_valid,
