...
case MODE_TRACKING:
  ...
  {
    TrackerController_Reset(&g.tracker);     // 清 PID 積分
    MotorControl_StopAll(&g.motor);          // 停下兩軸
  }
  else
  {
    cmd = TrackerController_Run(&g.tracker, &g.ldr.frame, period_ms);
...
| `PID_ERR_DEADBAND` | ↑ (例如 0.030) | 中央死區放大,偏少不動 |
...
| `PID_ERR_DEADBAND` | ↓ (例如 0.005) | 微小偏差也動 |
| `LDR_BASELINE_MARGIN` | ↓ | 校正後更敏感 |

### 追到目標但小範圍擺動

- 提高 `PID_ERR_DEADBAND`(主要)
- 降低 PID 的 `M*_KP_SMALL`(細節章節 [06_PID_控制邏輯.md](06_PID_控制邏輯.md))

### 追蹤太慢

- 提高 `M*_KP_MEDIUM` 與 `M*_KP_LARGE`
- 提高 `M*_RATE_LIMIT_HZ`
...
**選擇 B — 改通道映射**
[app_adc.c::AppAdc_Task()](../Core/Src/App/app_adc.c:38) 把 `raw[0]` 跟 `raw[1]` 對調,效果是水平翻轉。
要垂直翻轉就對調 `raw[2]` 跟 `raw[3]`。

**選擇 C — 改馬達 DIR**
[stepper_tmc2209.c::apply_speed()](../Core/Src/App/stepper_tmc2209.c) 把 `GPIO_PIN_RESET` 跟 `GPIO_PIN_SET` 對調 (只對某一軸)。
或改 [tracking_types.h](../Core/Inc/App/tracking_types.h) 在 PID 輸出後加負號(較笨)。

**選擇 D — TMC2209 GCONF**
GCONF bit3 (`shaft`) 設 1 翻轉該軸方向。詳見 [03_TMC2209_暫存器.md](03_TMC2209_暫存器.md#1-gconf-0x00--全域設定)。

推薦順序: A > D > C > B(改硬體最不會影響其他軟體邏輯)。
